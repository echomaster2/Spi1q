      ...
      <div className="relative z-10 space-y-6 md:space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[9px] md:text-[10px] font-black text-registry-teal uppercase tracking-[0.3em]">Phase 3: Cognitive Analytics</p>
          </div>
          <div className={`flex items-center space-x-2 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'} px-3 py-1.5 md:px-4 md:py-2 rounded-xl border`}>
            ...
            <span className={`text-[10px] md:text-xs font-black ${isDarkMode ? 'text-white' : 'text-slate-900'} uppercase italic`}>Peak State</span>
          </div>
        </div>
        ...
        <div className="flex items-end justify-between h-24 md:h-32 gap-1.5 md:gap-2">
          {data.map((val, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-2">
              ...
              />
              <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">D{i+1}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className={`p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'} rounded-2xl border`}>
            <div className={`flex items-center space-x-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} mb-1`}>
              ...
              <span className="text-[8px] font-black uppercase tracking-widest">Synaptic Depth</span>
            </div>
          ...
          <div className={`p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'} rounded-2xl border`}>
            <div className={`flex items-center space-x-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'} mb-1`}>
              ...
              <span className="text-[8px] font-black uppercase tracking-widest">Retention Rate</span>
            </div>
